...
/**
 * bitspay Api
 * A PHP-based bitspay client library with a focus on simplicity and ease of integration
 */
...
class bitspay
{
  ...
  const PRODUCTION_HOST = "api.bitspay.net";
  const PRODUCTION_DASHBOARD_HOST = "dashboard.bitspay.net";
  ...
  {
    bitspay::$API_MODE = $mode;
    return bitspay::getApiMode();
  }
  ...
  static public function getClient($token)
  {
    ...
    if (bitspay::getApiMode() == 'test')
    {
      $client -> options['host'] = bitspay::TEST_HOST;
      $client -> options['dashboard_host'] = bitspay::TEST_DASHBOARD_HOST;
    }
    return $client;
  }
  ...
  {
    $client = bitspay::getClient($token);
    $user = NULL;
    if (empty($id))
    {
 ...
 */
function bitspayAutoload($class_name)
{
  $dirs = array(
    __DIR__ . '/',
    __DIR__ . '/services/',
...
{
  spl_autoload_register('bitspayAutoload', TRUE, TRUE);
}
else
{
  spl_autoload_register('bitspayAutoload');
}
